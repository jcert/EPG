...
for i in integrator
    ...
    if EstimateFrom == 2
        ...
        x = [C_logit_payoff(g0,g0.r_star,0.0,i;η=myPBR_η_real)  for i=1:g0.NS ]
    else
    ...
    if integrator.t>1 && pre_C_choice_learned && length(D)>1 && q_converged(us)
        l(η;r) = begin
            s = exp.(inv(abs(η[1]))*r)        
            s/sum(s)
        end 
        ...
        myPBR_η_estimated = optimize(J,[4.0], autodiff = :forward).minimizer[1]
