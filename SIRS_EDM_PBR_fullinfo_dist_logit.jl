...
for g0.υ = υ_list
    ...
    prob = ODEProblem((du, u, p, t)->h_logit!(du,u,p,t;η=η_real),W ,[0.0,T],g0)
    ...
    plot!(f1,sol.t, sum(X.*rr,dims=1)', label="η=$(η_real),cost(t),υ=$(g0.υ)", legend = nothing)
end
plot!(size=plt_size)
...
xlabel!("days")
ylims!(-0.4,0.4)
savefig("images/logit.a.SIRS_EDM_cost_$(g0.c_star)_nu$(round(g0.υ,digits=1)).$(plt_ext)")
...
for g0.υ = υ_list
    ...
    Plots.plot!(f2,sol.t, (sol[1,:]./(g0.β'*X)')./(i_star), label="η=$(η_real),I,υ=$(g0.υ)",legend = nothing)
end
plot!(size=plt_size)
ylabel!(L"I(t)/I^*")
xlabel!("days")
...
savefig("images/logit.b.SIRS_EDM_I_ratio_$(g0.c_star)_nu$(round(g0.υ,digits=1)).$(plt_ext)")
...
for g0.υ = υ_list
    ...
    plot!(f3,sol, idxs=(3), label="η=$(η_real),x_1,υ=$(g0.υ)",legend = nothing)
end
plot!(size=plt_size)
ylabel!("x_1")
xlabel!("days")
savefig("images/logit.c.SIRS_EDM_x1_$(g0.c_star)_nu$(round(g0.υ,digits=1)).$(plt_ext)")
